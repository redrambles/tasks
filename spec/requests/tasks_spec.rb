...
RSpec.describe "/tasks", type: :request do
  ...
  let(:valid_attributes_status_false) {
    {title: "Read this book", due_date: 2.day.from_now, status: false}
  }
  ...
  describe "GET /index" do
    it "renders a list of completed tasks" do
      completed_task = Task.create!(valid_attributes_status_true)
      incomplete_task = Task.create!(valid_attributes_status_false)

      get tasks_url, params: {status: "completed"}
      expect(response).to be_successful
      expect(response.body).to include(completed_task.title)
      expect(response.body).not_to include(incomplete_task.title)
    end

    it "renders a list of incomplete tasks" do
      completed_task = Task.create!(valid_attributes_status_true)
      incomplete_task = Task.create!(valid_attributes_status_false)

      get tasks_url, params: {status: "incomplete"}
      expect(response).to be_successful
      expect(response.body).to include(incomplete_task.title)
      expect(response.body).not_to include(completed_task.title)
    end

    it "renders a list of both completed and incomplete tasks" do
      completed_task = Task.create!(valid_attributes_status_true)
      incomplete_task = Task.create!(valid_attributes_status_false)
